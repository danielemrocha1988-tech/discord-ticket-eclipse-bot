...
class TicketCog(commands.Cog):
    ...
    # ===================== MENU PRINCIPAL (1ª imagem) =====================
    @commands.command(name="setup_ticket")
    @commands.has_permissions(administrator=True)
    async def setup_ticket(self, ctx):
        """Envia o painel de criação de tickets (1ª imagem)"""
        embed = discord.Embed(
    ...
    async def create_ticket(self, interaction: discord.Interaction, category_name: str):
        ...
        # Mensagem inicial no canal (similar à 3ª imagem)
        embed = discord.Embed(
            title="🎫 **TICKET ABERTO**",
...
class TicketControlView(discord.ui.View):
    ...
    async def fechar_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self.is_staff(interaction.user) and interaction.user.id != self.user_id:
...
# ===================== INICIALIZAÇÃO =====================
# O token será lido do arquivo .env automaticamente
